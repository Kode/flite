...
class CanvasGraphics {
	static inline final SIN45 = 0.70710678118654752440084436210485;
	static inline final TAN22 = 0.4142135623730950488016887242097;
	static var allowSmoothing:Bool;
	static var bitmapFill:BitmapData;
	static var bitmapStroke:BitmapData;
	static var bitmapRepeat:Bool;
	static var bounds:Rectangle;
	static var fillCommands:DrawCommandBuffer = new DrawCommandBuffer();
	static var graphics:Graphics;
	static var hasFill:Bool;
	static var hasStroke:Bool;
	static var hitTesting:Bool;
	static var inversePendingMatrix:Matrix;
	static var pendingMatrix:Matrix;
	static var strokeCommands:DrawCommandBuffer = new DrawCommandBuffer();
	static var windingRule:CanvasWindingRule;
	static var context:CanvasRenderingContext2D;
	static var hitTestCanvas:CanvasElement;
	static var hitTestContext:CanvasRenderingContext2D;

	static function __init__() {
		setupCanvasContextCompatFunctions();
		hitTestCanvas = Browser.document.createCanvasElement();
		hitTestContext = hitTestCanvas.getContext("2d");
	}

	static function setupCanvasContextCompatFunctions() {
		var proto = (cast CanvasRenderingContext2D).prototype;
		if (proto.isPointInStroke == null) {
			proto.isPointInStroke = function(path, x, y) return false;
		}
		if (proto.isPointInPath == null) {
			proto.isPointInPath = function(path, x, y) return false;
		}
	}
	...
	public static function render(graphics:Graphics, pixelRatio:Float, smoothing:Bool) {
		graphics.__update();
		...
		if (dirty) {
			...
			CanvasGraphics.graphics = graphics;
			CanvasGraphics.allowSmoothing = smoothing;
			bounds = graphics.__bounds;
			...
			if (!graphics.__visible || graphics.__commands.length == 0 || bounds == null || width < 1 || height < 1) {
				...
				if (graphics.__canvas == null) {
					graphics.__canvas = Browser.document.createCanvasElement();
	...
	public static function renderMask(graphics:Graphics, context:CanvasRenderingContext2D) {
	...
	static function setSmoothing(smooth:Bool) {
