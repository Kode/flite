...
typedef UploadCallback = (side:UInt, level:Int, gpuFormat:ATFGPUFormat, width:Int, height:Int, data:UInt8Array)->Void;
...
enum abstract ATFFormat(Int) {
	var RGB888;
	var RGBA8888;
	var COMPRESSED; // JPEG-XR+LZMA & Block compression
	var RAW_COMPRESSED; // Block compression
	var COMPRESSED_ALPHA; // JPEG-XR+LZMA & Block compression with Alpha
	var RAW_COMPRESSED_ALPHA; // Block compression with Alpha
}
...
class ATFReader {
		...
		for (side in 0...sideCount) {
			for (level in 0...mipCount) {
				for (gpuFormat in 0...gpuFormats) {
					...
					if (blockLength > 0) {
						var bufferView = UInt8Array.fromBytes(data, data.position, blockLength);

						data.position += blockLength;

						uploadCallback(side, level, cast gpuFormat, width >> level, height >> level, bufferView);
					}
				}
			}
		}
	...
	static inline function __readUInt24(data:ByteArray):UInt {
		var value;
	...
	static inline function __readUInt32(data:ByteArray):UInt {
		var value;
