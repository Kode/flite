...
class SimpleButton extends InteractiveObject {
	...
	private override function __getCursor():MouseCursor {
		return (useHandCursor && !__ignoreEvent) ? BUTTON : null;
	}
	...
	private override function __renderCanvas(renderSession:CanvasRenderSession):Void {
		if (!__renderable || __worldAlpha <= 0 || __currentState == null)
			return;
	...
	override function __overrideTransforms(overrideTransform:Matrix) {
		...
		if (__currentState != null) {
			__currentState.__overrideTransforms(overrideTransform);
		}

		if (hitTestState != null && hitTestState != __currentState) {
			hitTestState.__overrideTransforms(overrideTransform);
		}
	}
