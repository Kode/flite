...
final class Graphics {
	...
	function __getBatchQuad(renderSession:GLRenderSession, alpha, colorTransform, blendMode):Quad {
		if (__batchQuadDirty) {
			...
			__batchQuad.texture = @:privateAccess __bitmap.__getTexture();
	...
	private function __update() {
		...
		var scaleX =
			if (parentTransform.b == 0)
				Math.abs(parentTransform.a)
			else
				Math.sqrt(parentTransform.a * parentTransform.a + parentTransform.b * parentTransform.b);

		var scaleY =
			if (parentTransform.c == 0)
				Math.abs(parentTransform.d)
			else
				Math.sqrt(parentTransform.c * parentTransform.c + parentTransform.d * parentTransform.d);
