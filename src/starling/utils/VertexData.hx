...
class VertexData {
	...
	public function clone(vertexID:Int = 0, numVertices:Int = -1):VertexData {
		...
		#else
		clone.mRawData = mRawData.subarray(vertexID * ELEMENTS_PER_VERTEX, numVertices * ELEMENTS_PER_VERTEX);
		#end
		return clone;
	}
	 ...
	 * instead of creating a new object. To use all vertices for the calculation, set
	 * 'numVertices' to '-1'. */
	public function getBoundsProjected(transformationMatrix:Matrix3D, camPos:Vector3D, vertexID:Int = 0, numVertices:Int = -1,
	...
	public function toString():String {
		...
		for (i in 0...numVertices) {
			...
			result += "  [Vertex " + i + ": " + "x=" + Std.string(position.x) + ", " + "y=" + Std.string(position.y) + ", " + "rgb="
				+ StringTools.hex(getColor(i)) + ", " + "a=" + Std.string(getAlpha(i)) + ", " + "u=" + Std.string(texCoords.x) + ", " + "v="
				+ Std.string(texCoords.y) + "]" + (i == numVertices - 1 ? "\n" : ",\n");
		}
	...
	private function get_rawData():Float32Array {
		return mRawData;
	}
	#end
}
