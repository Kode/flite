...
class Polygon {
	...
	public function addVertices(args:Array<Dynamic>):Void {
		...
		if (numArgs > 0) {
			if (Std.is(args[0], Point)) {
				...
				throw new ArgumentError("Invalid type: " + Type.getClassName(Type.getClass(args[0])));
		}
	}
	...
	public function copyToVertexData(target:VertexData, targetIndex:Int = 0):Void {
		...
		#else
		copyToArray(target.rawData, targetIndex * VertexData.ELEMENTS_PER_VERTEX, VertexData.ELEMENTS_PER_VERTEX - 2);
		#end
	}
	 ...
	 * 'stride' coordinates between each 'x, y' pair. */
	public function copyToVector(target:Vector<Float>, targetIndex:Int = 0, stride:Int = 0):Void {
		var numVertices:Int = this.numVertices;
	...
	public function toString():String {
		...
		for (i in 0...numPoints) {
			result += "  [Vertex " + i + ": " + "x=" + (Math.round(mCoords[i * 2] * 10) / 10) + ", " + "y=" + (Math.round(mCoords[i * 2 + 1] * 10) / 10)
				+ "]" + (i == numPoints - 1 ? "\n" : ",\n");
		}
	...
	public static function createRectangle(x:Float, y:Float, width:Float, height:Float):Polygon {
	...
	inline private static function isConvexTriangle(ax:Float, ay:Float, bx:Float, by:Float, cx:Float, cy:Float):Bool {
	...
	private static function isPointInTriangle(px:Float, py:Float, ax:Float, ay:Float, bx:Float, by:Float, cx:Float, cy:Float):Bool {
	...
	private static function areVectorsIntersecting(ax:Float, ay:Float, bx:Float, by:Float, cx:Float, cy:Float, dx:Float, dy:Float):Bool {
		...
		var s:Float = aby != 0.0 ? (cy - ay + t * cdy) / aby : (cx - ax + t * cdx) / abx;
	...
	private function get_isConvex():Bool {
		...
		if (numCoords < 6)
			...
			var i:Int = 0;
			while (i < numCoords) {
				if (!isConvexTriangle(mCoords[i], mCoords[i + 1], mCoords[(i + 2) % numCoords], mCoords[(i + 3) % numCoords], mCoords[(i + 4) % numCoords],
					mCoords[(i + 5) % numCoords])) {
					return false;
				}
				i += 2;
			}
