...
class FragmentFilter {
	...
	private inline static var STD_VERTEX_SHADER:String = "m44 op, va0, vc0 \n" + // 4x4 matrix transform to output space
		"mov v0, va1      \n"; // pass texture coordinates to fragment program

	/** The standard fragment shader code. It just forwards the texture color to the output. */
	private inline static var STD_FRAGMENT_SHADER:String = "tex oc, v0, fs0 <2d, clamp, linear, mipnone>"; // just forward texture color
	...
	private function renderPasses(object:DisplayObject, support:RenderSupport, parentAlpha:Float, intoCache:Bool = false):Image {
	...
	private function updatePassTextures(width:Float, height:Float, scale:Float):Void {
		...
		var needsUpdate:Bool = mPassTextures.length != numPassTextures
	 ...
	 */
	private function calculateBounds(object:DisplayObject, targetSpace:DisplayObject, scale:Float, intersectWithStage:Bool, resultRect:Rectangle,
		...
		if (!resultRect.isEmpty()) {
			...
			resultPotRect.setTo(resultRect.x, resultRect.y, PowerOfTwo.getNextPowerOfTwo(Std.int(minWidth * scale)) / scale,
