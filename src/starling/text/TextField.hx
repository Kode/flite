...
class TextField extends DisplayObjectContainer {
	...
	public function new(width:Int, height:Int, text:String, fontName:String = "_sans", fontSize:Float = 12, color:UInt = 0x0, bold:Bool = false) {
		super();
	...
	public function redraw():Void {
		if (mRequiresRedraw) {
			if (getBitmapFont(mFontName) != null)
	...
	private function renderText(scale:Float, resultTextBounds:Rectangle):BitmapData {
		...
		var textFormat:TextFormat = new TextFormat(mFontName, Std.int(mFontSize * scale), mColor, mBold, mItalic, mUnderline, null, null, hAlign);
		...
		var drawMatrix:Matrix = new Matrix(1, 0, 0, 1, filterOffset.x, filterOffset.y + Std.int(textOffsetY) - 2);
		#if flash
		var drawWithQualityFunc:Function = Reflect.getProperty(bitmapData, "drawWithQuality");
		...
		if (Reflect.isFunction(drawWithQualityFunc))
			Reflect.callMethod(bitmapData, drawWithQualityFunc, [sNativeTextField, drawMatrix, null, null, null, false, StageQuality.MEDIUM]);
		else
	...
	public override function hitTest(localPoint:Point, forTouch:Bool = false):DisplayObject {
		if (forTouch && (!visible || !touchable))
			return null;
		else if (mHitArea.containsPoint(localPoint) && hitTestMask(localPoint))
			return this;
		else
			return null;
	}
