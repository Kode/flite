...
class Tween extends EventDispatcher implements IAnimatable {
	...
	public function reset(target:Dynamic, time:Float, transition:Dynamic = "linear"):Tween {
		...
		if (mUpdateFuncs != null)
			mUpdateFuncs.length = 0;
		else
			mUpdateFuncs = new Vector<String->Float->Float->Void>();
	...
	/** @private */
	@:allow(starling) private static function fromPool(target:Dynamic, time:Float, transition:Dynamic = "linear"):Tween {
