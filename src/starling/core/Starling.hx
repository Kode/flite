...
class Starling extends EventDispatcher {
	...
	public function new(rootClass:Class<Dynamic>, stage:flash.display.Stage, viewPort:Rectangle = null, stage3D:Stage3D = null,
	...
	private function requestContext3D(stage3D:Stage3D, renderMode:Context3DRenderMode, profile:Dynamic):Void {
		...
		if (profile == "auto")
			profiles = [
				/*Context3DProfile.STANDARD_EXTENDED, Context3DProfile.STANDARD,*/
		...
		function requestNextProfile():Void {
			...
			try {
				Execute.execute(mStage3D.requestContext3D, [renderMode, currentProfile]);
			} catch (error:Error) {
	...
	public function nextFrame():Void {
		...
		if (passedTime < 0.0)
			passedTime = 1.0 / mNativeStage.frameRate;
	...
	public function render():Void {
		...
		mSupport.setProjectionMatrix(mViewPort.x < 0 ? -mViewPort.x / scaleX : 0.0, mViewPort.y < 0 ? -mViewPort.y / scaleY : 0.0,
			mClippedViewPort.width / scaleX, mClippedViewPort.height / scaleY, mStage.stageWidth, mStage.stageHeight, mStage.cameraPosition);
	...
	private function updateViewPort(forceUpdate:Bool = false):Void {
		...
		if (forceUpdate
			...
			if (!mShareContext) {
				...
				configureBackBuffer(Std.int(mClippedViewPort.width), Std.int(mClippedViewPort.height), mAntiAliasing, true, mSupportHighResolutions);
	...
	private function configureBackBuffer(width:Int, height:Int, antiAlias:Int, enableDepthAndStencil:Bool, wantsBestResolution:Bool = false):Void {
	...
	public function registerProgram(name:String, vertexShader:Dynamic, fragmentShader:Dynamic):Program3D {
		deleteProgram(name);
	...
	private function get_programs():Map<String, Program3D> {
	...
	private function get_backBufferWidth():Int {
		return Std.int(mClippedViewPort.width);
	}
	...
	private function get_backBufferHeight():Int {
		return Std.int(mClippedViewPort.height);
	}
	...
	private function get_broadcastKeyboardEvents():Bool {
