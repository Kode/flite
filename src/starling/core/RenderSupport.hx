...
class RenderSupport {
	...
	private function get_projectionMatrix():Matrix {
	...
	public function setRenderTarget(target:Texture, antiAliasing:Int = 0):Void {
		...
		if (target != null) {
			...
			Starling.current.context.setRenderToTexture(target.base, SystemUtil.supportsDepthAndStencil, antiAliasing);
		} else {
	...
	public static function _clear(rgb:UInt = 0, alpha:Float = 0.0):Void {
		Starling.current.context.clear(Color.getRed(rgb) / 255.0, Color.getGreen(rgb) / 255.0, Color.getBlue(rgb) / 255.0, alpha);
	}
	...
	public static function assembleAgal(vertexShader:String, fragmentShader:String, resultProgram:Program3D = null):Program3D {
		...
		resultProgram.upload(sAssembler.assemble(Context3DProgramType.VERTEX, vertexShader),
	...
	public static function getTextureLookupFlags(format:Context3DTextureFormat, mipMapping:Bool, repeat:Bool = false, smoothing:String = "bilinear"):String {
	...
	inline function get_batcher()
		return @:privateAccess Starling.current.context.__renderSession.batcher;
