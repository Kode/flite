...
class TextureOptions {
	...
	public function new(scale:Float = 1.0, mipMapping:Bool = false, format:Context3DTextureFormat = BGRA, repeat:Bool = false) {
		mScale = scale;
		mFormat = format;
	...
	private function set_optimizeForRenderToTexture(value:Bool):Bool {
		return mOptimizeForRenderToTexture = value;
	}
	...
	private function get_onReady():Void->Void {
		return mOnReady;
	}

	private function set_onReady(value:Void->Void):Void->Void {
		return mOnReady = value;
	}
}
